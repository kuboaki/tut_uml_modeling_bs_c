= UML初学者向けチュートリアル: モデルを使ってソフトウェアを開発しよう（C言語版）
...
* https://github.com/ChangeVision/tutorial-bowling-score-modeling_c/releases[リリースページ]

tut_uml_modeling_bs_c.pdf:: 電子ブックの体裁のPDF版です。
tut_uml_modeling_bs_c_html.zip:: ブラウザで閲覧するHTML版です。Zipファイルを展開すると、`tut_uml_modeling_bs_c_html` フォルダにファイルが展開されます。ウェブブラウザから `tut_uml_modeling_bs_c_full.html` か `tut_uml_modeling_bs_c.html` を参照します。前者は全体を1つのページとしてまとめた形式、後者は章ごとに別ページに分割した形式です。
